...
// -----------------------------
// Auto-detect Location & Fill Road Name
// -----------------------------
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
      ...
      try {
        ...
        if (data.address) {
          ...
          document.getElementById("locationText").value = road || data.display_name || "";
        }
      } catch (err) {
...
function generateTracking() {
  let d = new Date(), rand = Math.floor(Math.random() * 1000);
  return "RW-" + d.getFullYear() +
    (d.getMonth() + 1).toString().padStart(2, "0") +
    d.getDate().toString().padStart(2, "0") + "-" +
    d.getHours() + d.getMinutes() + d.getSeconds() + "-" + rand;
}
...
async function submitReport() {
  if (lat === 0 || lng === 0) { alert("Please select location on the map or use auto-detect"); return; }
...
// -----------------------------
// Send Report to Google Sheet Web App
// -----------------------------
